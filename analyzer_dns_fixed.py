...
# Path configuration
RAW_PATH = os.path.join("results", "raw_packets.json")  # Path to raw packet data
OUT_PATH = os.path.join("results", "dns_analysis.json")  # Output path for analysis results
LOG_PATH = os.path.join("logs", "dns_analyzer.log")  # Log file path
...
def load_raw() -> List[Dict[str, Any]]:
    ...
    Returns:
        List of packet data or empty list if error occurs.
    """
    try:
        logger.debug(f"Loading raw packets from {RAW_PATH}")
        ...
        try:
            ...
            if p.get("protocol") in ("UDP", "TCP") and (p.get("dst_port") == 53 or p.get("src_port") == 53):
                ...
                try:
                    ...
                    # In a real implementation, you would use a proper DNS parser
                    dns_data = {
